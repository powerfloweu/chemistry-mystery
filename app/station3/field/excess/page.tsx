...
export default function ExcessTrial() {
  ...
  const lines = useMemo(
    () => [
      "FIELD SITE 3 // EXCESS",
      "Stoichiometric imbalance perturbs equilibrium.",
      "Certify the direction that consumes the added component.",
    ],
    []
  );
  ...
  const handleChoice = (choice: "consumption" | "reverse") => {
    if (certified) return;
    setError(false);
      ...
      if (choice === "consumption" || isTestInput) {
      setCertified(true);
      setField("s3_excess", true);
    } else {
      setError(true);
      ...
      <BasicShell title="Exploration" subtitle="Station 3 • Field Verification">
        <div className="space-y-6">
          ...
          {!certified && (
            <Folio
              label="PERTURBATION"
              title="Excess reagent introduced"
              note="Identify the equilibrium response to stoichiometric excess."
            >
              <div className="space-y-4">
                <p className="text-sm leading-relaxed text-slate-800">
                  Upon introduction of an excess component, which direction is favored at equilibrium?
                </p>
                {isDevMode() && (
                ...
                <div className="grid grid-cols-2 gap-3">
                  <Button
                    variant="secondary"
                    onClick={() => handleChoice("consumption")}
                    className="w-full"
                  >
                    Toward consumption of the excess
                  </Button>
                  <Button
                    variant="secondary"
                    onClick={() => handleChoice("reverse")}
                    className="w-full"
                  >
                    Toward reversal
                  </Button>
              ...
              <div className="space-y-4">
                <div className="p-3 rounded-lg bg-white/50 border border-slate-900/10">
                  <p className="text-sm font-medium text-emerald-800">
                    FIELD SITE 3 — CERTIFIED
                  </p>
                  <p className="text-xs text-slate-700/70 mt-1">
                    Excess reagent verification complete. Return to the Station 3 hub.
                  </p>
                </div>
