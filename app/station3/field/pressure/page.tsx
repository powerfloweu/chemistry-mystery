...
export default function PressureTrial() {
  ...
  const lines = useMemo(
    () => [
      "FIELD SITE 2 // PRESSURE",
      "Mechanical compression biases equilibrium toward reduced volume.",
      "Certify the configuration that survives applied pressure.",
    ],
    []
  );
  ...
  const handleChoice = (choice: "fewer" | "more") => {
    if (certified) return;
    setError(false);
        ...
        <div className="space-y-6">
          ...
          {!certified && (
            <Folio
              ...
              note="Choose the configuration favored under compression."
            >
              <div className="space-y-4">
                <p className="text-sm leading-relaxed text-slate-800">
                  Under increased pressure, which configuration is favored at equilibrium?
                </p>
                {isDevMode() && (
                ...
                <div className="grid grid-cols-2 gap-3">
                  <Button
                    variant="secondary"
                    onClick={() => handleChoice("fewer")}
                    className="w-full"
                  >
                    Fewer accessible states
                  </Button>
                  <Button
                    variant="secondary"
          ...
          {certified && (
            <Folio
              ...
              note="Proceed back to the hub."
            >
              <div className="space-y-4">
                <div className="p-3 rounded-lg bg-white/50 border border-slate-900/10">
                  <p className="text-sm font-medium text-emerald-800">
                    FIELD SITE 2 — CERTIFIED
                  </p>
                  <p className="text-xs text-slate-700/70 mt-1">
                    Pressure verification complete. Return to the Station 3 hub.
                  </p>
                </div>
